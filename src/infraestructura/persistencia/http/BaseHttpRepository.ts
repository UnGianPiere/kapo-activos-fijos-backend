...
export abstract class BaseHttpRepository<T> {
  ...
  protected async getClient(serviceName: string, fallbackServiceName: string = 'inacons-backend'): Promise<GraphQLClient> {
    ...
    try {
      ...
      // Configuración especial para servicios externos (mayor timeout)
      const config = serviceName === 'inacons-backend' || fallbackServiceName === 'inacons-backend'
        ? {
            timeout: 30000, // 30 segundos para conexiones al monolito
            maxConcurrent: 5,
            retry: {
              maxRetries: 2,
              baseDelay: 1000,
        ...
        : undefined;
      this.client = new GraphQLClient(url, config);
      return this.client;
    } catch {
      const url = await this.serviceRegistry.getServiceUrl(fallbackServiceName);
      // Configuración especial para servicios externos (mayor timeout)
      const config = fallbackServiceName === 'inacons-backend'
        ? {
            timeout: 30000, // 30 segundos para conexiones al monolito
            maxConcurrent: 5,
            retry: {
